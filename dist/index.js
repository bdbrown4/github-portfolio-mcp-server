...
async function handleListRepos(args) {
    const repos = await ghFetch(`/users/${GITHUB_USERNAME}/repos?per_page=100&sort=updated`);
...
async function handleGetRepoDetails(args) {
    const repo = await ghFetch(`/repos/${GITHUB_USERNAME}/${args.repo_name}`);
    let readme = "(No README found)";
    try {
        const readmeRes = await fetch(`${GITHUB_API}/repos/${GITHUB_USERNAME}/${args.repo_name}/readme`, { headers: { ...headers, Accept: "application/vnd.github.v3.raw" } });
        if (readmeRes.ok)
...
async function handleGetLanguages(args) {
    const languages = await ghFetch(`/repos/${GITHUB_USERNAME}/${args.repo_name}/languages`);
    const total = Object.values(languages).reduce((a, b) => a + b, 0);
    return JSON.stringify(Object.entries(languages).map(([lang, bytes]) => ({
        language: lang, bytes,
...
async function handleSearchRepos(args) {
    const repos = await ghFetch(`/users/${GITHUB_USERNAME}/repos?per_page=100`);
    const q = args.query.toLowerCase();
    const matches = repos.filter((r) => r.name.toLowerCase().includes(q) ||
...
const TOOL_REGISTRY = {
    list_repos: (a) => handleListRepos(a),
    get_repo_details: (a) => handleGetRepoDetails(a),
    get_languages: (a) => handleGetLanguages(a),
    get_profile: () => handleGetProfile(),
    search_repos: (a) => handleSearchRepos(a),
...
}, async (args) => ({ content: [{ type: "text", text: await handleListRepos(args) }] }));
...
}, async (args) => ({ content: [{ type: "text", text: await handleGetRepoDetails(args) }] }));
...
server.registerTool("get_tech_stack_summary", {
    ...
    inputSchema: {},
}, async () => ({ content: [{ type: "text", text: await handleGetTechStackSummary() }] }));
server.registerResource("profile", `github://${GITHUB_USERNAME}/profile`, { description: `Public GitHub profile for ${GITHUB_USERNAME}`, mimeType: "application/json" }, async () => ({
    contents: [{ uri: `github://${GITHUB_USERNAME}/profile`, mimeType: "application/json", text: await handleGetProfile() }],
}));
server.registerResource("repo-readme", new ResourceTemplate("github://{owner}/{repo}/readme", { list: undefined }), { description: "README content for a specific repository", mimeType: "text/markdown" }, async (uri, { owner, repo }) => {
    const readmeRes = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/readme`, { headers: { ...headers, Accept: "application/vnd.github.v3.raw" } });
    return {
