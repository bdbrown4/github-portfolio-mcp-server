...
server.registerTool("list_repos", {
    ...
    description: `List public repositories for the portfolio owner (${GITHUB_USERNAME}). ` +
...
server.registerTool("get_repo_details", {
    ...
    description: "Get detailed information about a specific repository including its README content.",
    inputSchema: {
        repo_name: z.string().describe("The repository name (e.g. 'crypto-web-component')"),
    },
}, async ({ repo_name }) => {
    const repo = await ghFetch(`/repos/${GITHUB_USERNAME}/${repo_name}`);
    // Try to fetch the README
    let readme = "(No README found)";
    try {
        const readmeRes = await fetch(`${GITHUB_API}/repos/${GITHUB_USERNAME}/${repo_name}/readme`, { headers: { ...headers, Accept: "application/vnd.github.v3.raw" } });
        if (readmeRes.ok) {
...
server.registerTool("get_languages", {
    ...
    description: "Get the programming languages used in a specific repository, with byte counts.",
    inputSchema: {
        repo_name: z.string().describe("The repository name"),
    },
}, async ({ repo_name }) => {
    const languages = await ghFetch(`/repos/${GITHUB_USERNAME}/${repo_name}/languages`);
...
server.registerTool("search_repos", {
    ...
    description: `Search ${GITHUB_USERNAME}'s repositories by keyword. Matches against repo name, description, and topics.`,
    inputSchema: {
...
server.registerTool("get_tech_stack_summary", {
    ...
    description: `Aggregate the programming languages and technologies used across all of ${GITHUB_USERNAME}'s repositories into a summary.`,
    inputSchema: {},
}, async () => {
    const repos = await ghFetch(`/users/${GITHUB_USERNAME}/repos?per_page=100`);
